<script context="module">
  // This is all from https://github.com/Skayo/svelte-infinite-loading
  ...
  const scrollBarStorage = {
    ...
    restore(element) {
      ...
      if (typeof target[this.key] === 'number') {
        target.scrollTop =
          target.scrollHeight - target[this.key] + target.scrollTop;
      }
      this.remove(target);
    },
    remove(element) {
...
<script>
  ...
  function getCurrentDistance() {
    ...
    if (direction === 'top') {
      distance =
        typeof scrollParent.scrollTop === 'number'
          ? scrollParent.scrollTop
          : scrollParent.pageYOffset;
    } else {
      const infiniteElementOffsetTopFromBottom =
        thisElement.getBoundingClientRect().top;
      const scrollElementOffsetTopFromBottom =
        scrollParent === window
          ? window.innerHeight
          : scrollParent.getBoundingClientRect().bottom;
      distance =
        infiniteElementOffsetTopFromBottom - scrollElementOffsetTopFromBottom;
    }
    return distance;
  }
...
<div
  class="infinite-loading-container w-full col-span-full"
  ...
  {#if showNoResults}
    <div class="infinite-status-promp text-5xl font-thin">
      <slot name="noResults">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          class="h-32 w-32 mr-auto ml-auto"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor">
          <path
  ...
  {#if showNoMore}
    <div class="infinite-status-prompt font-thin mt-8">
      <slot name="noMore">That's all, folks!</slot>
    </div>
  {/if}

  {#if showError}
    <div class="infinite-status-prompt text-5xl font-thin">
